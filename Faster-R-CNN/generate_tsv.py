...
def get_detections_from_im(fasterRCNN, classes, im_file, image_id, bbox, args, conf_thresh=0.2):
    ...
    objects = np.zeros([bboxes.shape[0],1])
    num_boxes_int = bboxes.shape[0]

    # initilize the tensor holder here.
    im_data = torch.FloatTensor(1)
    im_info = torch.FloatTensor(1)
    num_boxes = torch.LongTensor([num_boxes_int])
    ...
    #load images
    # 4차원 이미지 3차원으로 변경 - ghjin
    read_im_file = imread(im_file)
    if read_im_file.shape[2] == 4:
      read_im_file = read_im_file[:, :, :3]
      
    im_in = np.array(read_im_file)
    ...
    bboxes = bboxes * im_scales[0]
    gt_boxes = np.concatenate((objects, bboxes), axis = 1)
    ...
    temp_boxes = np.zeros((boxes.size()[0], 4))
    for i, v in enumerate(boxes):
      temp_boxes[i] = np.array(v.cpu())
    ...
    return {
        ...
        'num_boxes': num_boxes_int,
        'boxes': base64.b64encode(temp_boxes),
        'features': base64.b64encode((pooled_feat.cpu()).detach().numpy()),
    }
...
def generate_tsv(outfile, image_ids, args):
    ...
    wanted_ids = set([int(image_id) for _, image_id, _ in image_ids])
    found_ids = set()
    ...
    if len(missing) > 0:
        ...
        with open(outfile, 'a+') as tsvfile:
            ...
            _t = {'misc' : Timer()}
            count = 0
            for im_file,image_id, bbox in image_ids:
                if int(image_id) in missing:
                    _t['misc'].tic()
                    writer.writerow(get_detections_from_im(fasterRCNN, classes, im_file, image_id, bbox, args))
                    _t['misc'].toc()
                    if (count % 100) == 0:
...
if __name__ == '__main__':
    ...
    # load json
    with open('../BAN-KVQA/data/train_object_anno.json', 'r', encoding='utf-8') as file:
      train_anno = json.load(file)
    with open('../BAN-KVQA/data/val_object_anno.json', 'r', encoding='utf-8') as file:
      val_anno = json.load(file)
    with open('../BAN-KVQA/data/test_object_anno.json', 'r', encoding='utf-8') as file:
      test_anno = json.load(file)

    # trans annotation
    train_image_ids = [[i['image_file_path'], i['image_id'], i['bbox']] for i in train_anno]
    val_image_ids = [[i['image_file_path'], i['image_id'], i['bbox']] for i in val_anno]
    test_image_ids = [[i['image_file_path'], i['image_id'], i['bbox']] for i in test_anno]
    
    # output path
    train_output = args.outfile.replace('.tsv', '_train.tsv')
    val_output = args.outfile.replace('.tsv', '_val.tsv')
    test_output = args.outfile.replace('.tsv', '_test.tsv')
    
    # make tsv
    generate_tsv(train_output, train_image_ids, args)
    generate_tsv(val_output, val_image_ids, args)
    generate_tsv(test_output, test_image_ids, args)
