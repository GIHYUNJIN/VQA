...
def _load_kvqa(dataroot, name, img_id2val):
    ...
    question_path = os.path.join(
        # os.path.join('data/custom/annotations/%s_qa_anno.json' % name))
        os.path.join('data/%s_qa_anno.json' % name))
    # questions = sorted(json.load(open(question_path, encoding='utf-8')), key=lambda x: x['image'])
    questions = sorted(json.load(open(question_path, encoding='utf-8')), key=lambda x: x['question_id']) # change - ghjin
    idx2type = None
    type2idx = None
...
class KvqaFeatureDataset(Dataset):
    def __init__(self, split, dictionary, dataroot='data', tokenizer='sp'):
        ...
        ans2label_path = os.path.join(dataroot, 'cache', 'trainval_ans2label.pkl') # change - ghjin
        ...
        self.img_id2idx = cPickle.load(
            open('data/%s_imgid2idx.pkl' % split, # change - ghjin
                 'rb'))

        h5_path = 'data/%s_kvqa.hdf5' % split # change - ghjin
    ...
    def __getitem__(self, index):
        ...
        if None != answer:
            ...
            return features, spatials, question, target, entry['answerable'], entry['answer_type'], question_id # q_id - ghjin
        else:
