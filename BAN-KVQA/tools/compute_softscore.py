...
def compute_target(answers_dset, ans2label, split, cache_root='data/cache'):
    ...
    ***answers_dset should be preprocessed***
    ...
    utils.create_dir(cache_root)
    cache_file = os.path.join(cache_root, split + '_target.pkl')
...
if __name__ == '__main__':
    ...
    answer_file = os.path.join(dataroot, 'train_qa_anno.json')
    with open(answer_file, encoding='utf-8') as f:
        train_answers = json.load(f)

    answer_file = os.path.join(dataroot, 'val_qa_anno.json')
    with open(answer_file, encoding='utf-8') as f:
